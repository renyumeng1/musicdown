...
class HorizontalTextTabBar(QTabBar):
    ...
    def tabSizeHint(self, index):
        ...
        for i in range(self.count()):
            max_text_width = max(
                max_text_width, metrics.horizontalAdvance(self.tabText(i) or "")
            )
...
class WorkerThread(QThread):
    ...
    async def _download_single_song(self):
        ...
        quality_requested = (
            result.get("quality_requested", filetype) if result else filetype
        )
    ...
    async def _batch_search_and_download(self):
        ...
        for i, song in enumerate(songs):
            ...
            if search_result and search_result.get("songs"):
                ...
                singer_text = self._get_singer_names(
                    (song_info or {}).get("singer", [])
                )
    ...
    async def _search_playlist_link_songs(self):
        ...
        self.update_signal.emit(
            {"type": "playlist_link_songs_details", "data": detailed_songs}
        )
    ...
    async def _search_playlist_link_songs_concurrent(self):
        ...
        # 创建所有搜索任务
        tasks = [search_single_song(i, song_str) for i, song_str in enumerate(songs)]
        ...
        self.update_signal.emit(
            {"type": "playlist_link_search_complete", "total": total}
        )
    ...
    def _get_singer_names(self, singers):
        ...
        if isinstance(singers, list):
            return ", ".join(
                [
                    (
                        clean_html_tags(s.get("name", str(s)))
                        if isinstance(s, dict)
                        else clean_html_tags(str(s))
                    )
                    for s in singers
                ]
            )
...
class QQMusicDownloaderGUI(QMainWindow):
    ...
    def initUI(self):
        ...
        settings_outer.setContentsMargins(24, 24, 24, 24)
        settings_outer.setSpacing(20)
        settings_outer.setAlignment(Qt.AlignmentFlag.AlignTop)

        # ── 卡片 1：存储位置 ──
        path_card = QWidget()
        path_card.setObjectName("card")
        path_card.setAttribute(Qt.WidgetAttribute.WA_StyledBackground, True)
        path_card_layout = QVBoxLayout(path_card)
        path_card_layout.setContentsMargins(20, 16, 20, 16)
        path_card_layout.setSpacing(10)

        path_title = QLabel("存储位置")
        ...
        settings_outer.addWidget(path_card)

        # ── 卡片 2：音质偏好 ──
        quality_card = QWidget()
        quality_card.setObjectName("card")
        quality_card.setAttribute(Qt.WidgetAttribute.WA_StyledBackground, True)
        quality_card_layout = QVBoxLayout(quality_card)
        quality_card_layout.setContentsMargins(20, 16, 20, 16)
        quality_card_layout.setSpacing(12)

        quality_title = QLabel("音质偏好")
        quality_title.setObjectName("sectionTitle")
        quality_card_layout.addWidget(quality_title)

        quality_desc = QLabel("选择下载歌曲时优先使用的音质格式")
        quality_desc.setObjectName("mutedLabel")
        quality_card_layout.addWidget(quality_desc)

        # 标准音质行
        standard_row = QHBoxLayout()
        standard_row.setSpacing(16)
        self.quality_m4a = QRadioButton("M4A")
        self.quality_128 = QRadioButton("MP3 128k")
        self.quality_320 = QRadioButton("MP3 320k")
        self.quality_flac = QRadioButton("FLAC 无损")
        standard_row.addWidget(self.quality_m4a)
        standard_row.addWidget(self.quality_128)
        standard_row.addWidget(self.quality_320)
        standard_row.addWidget(self.quality_flac)
        standard_row.addStretch()
        quality_card_layout.addLayout(standard_row)

        # 高品质行
        hires_row = QHBoxLayout()
        hires_row.setSpacing(16)
        self.quality_ATMOS_51 = QRadioButton("臻品音质 2.0")
        self.quality_ATMOS_2 = QRadioButton("臻品全景声 2.0")
        self.quality_MASTER = QRadioButton("臻品母带 2.0")
        hires_row.addWidget(self.quality_ATMOS_51)
        hires_row.addWidget(self.quality_ATMOS_2)
        hires_row.addWidget(self.quality_MASTER)
        hires_row.addStretch()
        quality_card_layout.addLayout(hires_row)
        ...
        selected_quality = quality_map.get(self._saved_quality, self.quality_320)
        ...
        settings_outer.addWidget(quality_card)

        # ── 卡片 3：下载性能 ──
        perf_card = QWidget()
        perf_card.setObjectName("card")
        perf_card.setAttribute(Qt.WidgetAttribute.WA_StyledBackground, True)
        perf_card_layout = QVBoxLayout(perf_card)
        perf_card_layout.setContentsMargins(20, 16, 20, 16)
        perf_card_layout.setSpacing(10)

        perf_title = QLabel("下载性能")
        perf_title.setObjectName("sectionTitle")
        perf_card_layout.addWidget(perf_title)

        perf_row = QHBoxLayout()
        perf_row.setSpacing(10)
        perf_label = QLabel("同时下载数量")
        perf_row.addWidget(perf_label)

        self.concurrent_spinbox = QSpinBox()
        self.concurrent_spinbox.setRange(1, 10)
        self.concurrent_spinbox.setValue(int(self.concurrent_downloads or 5))
        self.concurrent_spinbox.setFixedWidth(100)
        self.concurrent_spinbox.setToolTip(
            "同时下载歌曲的数量，数值越大速度越快但占用更多带宽"
        )
        self.concurrent_spinbox.valueChanged.connect(self.save_config)
        perf_row.addWidget(self.concurrent_spinbox)

        perf_hint = QLabel("建议 3 ~ 6，过高可能导致限速")
        perf_hint.setObjectName("mutedLabel")
        perf_row.addWidget(perf_hint)
        perf_row.addStretch()
        perf_card_layout.addLayout(perf_row)

        settings_outer.addWidget(perf_card)
        ...
        self.download_table.setColumnWidth(6, 120)
        ...
        self.download_table.verticalHeader().setDefaultSectionSize(40)
        ...
        self.clear_all_downloads_btn = QPushButton("清除全部")
        self.clear_all_downloads_btn.clicked.connect(self.clear_all_downloads)
        progress_layout.addWidget(self.clear_all_downloads_btn)
        ...
        self.playlist_link_input.returnPressed.connect(self.get_playlist_from_link)
        ...
        self.playlist_link_table.setColumnWidth(2, 150)
        self.playlist_link_table.setColumnWidth(3, 130)
        self.playlist_link_table.setColumnWidth(5, 320)
        ...
        self.playlist_link_table.verticalHeader().setDefaultSectionSize(40)
        self.playlist_link_table.setColumnWidth(0, 28)
        ...
        self.select_all_link_btn.clicked.connect(self.select_all_playlist_link_songs)
        ...
        self.batch_download_link_btn.clicked.connect(self.batch_download_from_link)
        ...
        daily_header.setSectionResizeMode(QHeaderView.ResizeMode.Interactive)
        ...
        self.daily_table.setColumnWidth(2, 150)
        self.daily_table.setColumnWidth(3, 130)
        self.daily_table.setColumnWidth(5, 320)
        ...
        self.daily_table.verticalHeader().setDefaultSectionSize(40)
        self.daily_table.setColumnHidden(0, False)
        self.daily_table.setColumnWidth(0, 28)
    ...
    def handle_tab_change(self, index):
        ...
        if self.tabs.widget(index) != self.daily_tab:
            return
        if (
            not getattr(self, "_daily_auto_refresh_done", False)
            or self.daily_table.rowCount() == 0
        ):
    ...
    def check_login_status(self):
        ...
        try:
            ...
            QMessageBox.warning(self, "错误", f"检查登录状态时发生错误: {str(e)}")
    ...
    def _create_download_cell_widget(
    ...
    def _fill_song_row(self, row, song, song_list):
        ...
        # 下载按钮
        download_cell, download_btn = self._create_download_cell_widget(
            self.result_table
        )
    ...
    def _fill_album_row(self, row, album):
        ...
        view_btn.clicked.connect(lambda _, mid=album_mid: self.get_album_songs(mid))
    ...
    def _fill_playlist_row(self, row, playlist):
        ...
        view_btn = QPushButton("查看歌曲")
        view_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        view_btn.clicked.connect(lambda _, pid=int(playlist_id): self.get_playlist(pid))
        self.result_table.setCellWidget(row, 3, view_btn)
    ...
    def _check_format_availability(self, song, requested_format):
        ...
        def _is_available(fmt: str) -> bool:
            if fmt in {"ATMOS_51", "ATMOS_2", "MASTER"}:
                ...
                return bool(
                    idx is not None and len(size_new) > idx and size_new[idx] > 0
                )
        ...
        fallback_names = " / ".join(
            [self._get_format_display_name(c) for c in candidates[1:] if c]
        )
        return (
            False,
            f"该歌曲不支持{self._get_format_display_name(requested)}格式，且其它音质（{fallback_names}）也不可用",
        )
    ...
    def _format_quality_cell(
        self, quality_requested: str | None, quality_used: str | None
    ) -> str:
    ...
    def _quality_available_from_file(file_info: dict, quality: str) -> bool:
        ...
        if q == "m4a":
            return any(
                int(file_info.get(k, 0) or 0) > 0
                for k in ("size_192aac", "size_96aac", "size_48aac")
            )
        if q == "128":
        ...
        if q in {"MASTER", "ATMOS_2", "ATMOS_51"}:
            ...
            idx = idx_map.get(q)
            return bool(
                idx is not None and len(size_new) > idx and int(size_new[idx] or 0) > 0
            )

        return False

    def _predict_quality_for_song(
        self, song_info: dict, requested_quality: str
    ) -> str | None:
    ...
    def _confirm_quality_plan(
        self, songs: list[dict], requested_quality: str
        ...
        if unknown_lines:
            text_parts.append(
                f"无法预判（{len(unknown_lines)} 首）：下载时自动选择最高可用音质"
            )
    ...
    def _get_album_singer_name(self, album):
        ...
        if album.get("singer_list"):
            names = [
                clean_html_tags(s.get("name", str(s))) for s in album["singer_list"]
            ]
            return ", ".join(names)
    ...
    def _get_album_publish_time(self, album):
        """获取专辑发行时间"""
        return (
            album.get("publish_date")
            or album.get("publicTime")
            or album.get("public_time")
            or album.get("time")
            or ""
        )
    ...
    def _get_playlist_song_count(self, playlist):
        ...
        return (
            ...
            or playlist.get("song_count")
            or playlist.get("song_num")
            or playlist.get("total")
            or 0
        )
    ...
    def _get_playlist_id(self, playlist):
        """获取歌单ID"""
        return (
            playlist.get("dissid") or playlist.get("id") or playlist.get("disstid") or 0
        )

    def get_album_songs(self, album_mid):
        """获取专辑歌曲"""
        self.current_worker = WorkerThread(
            "get_album_songs", api=self.api, params={"album_mid": album_mid}
        )
    ...
    def _setup_album_action_buttons(self, songs):
        ...
        self.download_album_btn.clicked.connect(lambda: self.batch_download(songs))
    ...
    def download_song(self, song_info):
        ...
        self.download_table.setItem(row, 0, QTableWidgetItem(song_info["name"]))
        self.download_table.setItem(
            row,
            1,
            QTableWidgetItem(self._get_singer_names(song_info.get("singer", []))),
        )
        self.download_table.setItem(
            row,
            2,
    ...
    def batch_download(self, songs=None):
        ...
        for i, (song, predicted_quality) in enumerate(planned):
            ...
            self.download_table.setItem(
                ...
                2,
                QTableWidgetItem(
                    self._format_quality_cell(filetype, predicted_quality)
                ),
            )
            self.download_table.setCellWidget(row, 3, self._create_table_progress_bar())
            self.download_table.setItem(row, 4, QTableWidgetItem("等待下载..."))
            self.download_table.setItem(row, 5, QTableWidgetItem(""))
            self._add_download_delete_button(row)
        ...
        self.current_worker.update_signal.connect(self.handle_worker_update)
        self.current_worker.error_signal.connect(self.handle_worker_error)
        self.current_worker.progress_signal.connect(self.handle_progress_update)
        self.current_worker.start()
    ...
    def _add_download_delete_button(self, row: int):
        """为下载记录行添加删除按钮。"""
        container = QWidget()
        container.setStyleSheet("background: transparent;")
        ...
        del_btn.clicked.connect(lambda _, r=row: self._remove_download_row(r))
        layout.addWidget(del_btn)
        self.download_table.setCellWidget(row, 6, container)
    ...
    def _remove_download_row(self, row: int):
        ...
        if 0 <= row < self.download_table.rowCount():
            ...
            for r in range(self.download_table.rowCount()):
                self._add_download_delete_button(r)

    def clear_completed_downloads(self):
        """清除已完成（下载成功/下载失败）的下载记录。"""
        rows_to_remove = []
        for row in range(self.download_table.rowCount()):
            status_item = self.download_table.item(row, 4)
            if status_item and status_item.text() in ("下载成功", "下载失败"):
    ...
    def update_download_record(self, data):
        ...
        for row in range(self.download_table.rowCount()):
            ...
            if (
                song_name_item
                and song_name_item.text() == data["song_name"]
                and singer_item
                and singer_item.text() == data.get("singer", "")
                and status_item
                and status_item.text() in ["正在下载...", "等待下载..."]
            ):

                progress_bar = self._get_download_progress_bar(row)
                # 更新状态和路径
                if data["success"]:
                    self.download_table.setItem(row, 4, QTableWidgetItem("下载成功"))
                    self.download_table.setItem(row, 5, QTableWidgetItem(data["path"]))
                    progress_bar.setValue(100)
                else:
                    self.download_table.setItem(row, 4, QTableWidgetItem("下载失败"))
                ...
                if quality_used:
                    self.download_table.setItem(
                        ...
                        2,
                        QTableWidgetItem(
                            self._format_quality_cell(quality_requested, quality_used)
                        ),
                    )
    ...
    def update_download_progress(self, data):
        ...
        for row in range(self.download_table.rowCount()):
            ...
            if (
                ...
                and singer_item.text() == data.get("singer", "")
                and status_item
                and status_item.text() in ["等待下载...", "正在下载..."]
            ):
    ...
    def update_download_file_progress(self, data):
        ...
        for row in range(self.download_table.rowCount()):
            ...
            if (
                song_name_item
                and song_name_item.text() == song_name
                and singer_item
                and singer_item.text() == singer
            ):
                progress_bar = self._get_download_progress_bar(row)
    ...
    async def search_song_for_download(self, song_name, singer_name):
        ...
        result = await self.api.search(query, 1, 1)  # 限制为1个结果  # 页码
    ...
    def _get_playlist_original_songs_attr(self, source):
        return (
            "daily_original_songs"
            if source == "daily"
            else "playlist_link_original_songs"
        )
    ...
    def get_playlist_from_link(self):
        ...
        # 启动线程获取歌单
        self.current_worker = WorkerThread(
            "get_playlist_from_link", api=self.api, params={"url": link}
        )
    ...
    def handle_playlist_link_result(self, data):
        ...
        source = getattr(self, "_active_playlist_fetch_source", "") or getattr(
            self, "_last_playlist_source", "link"
        )
        ...
        if songs and isinstance(songs, list) and isinstance(songs[0], dict):
            ...
            # 若是“一键下载每日推荐”，则自动触发下载
            if (
                self._auto_download_after_playlist_fetch
                and self._last_playlist_source == "daily"
            ):
                self._auto_download_after_playlist_fetch = False
    ...
    def _render_playlist_link_songs(self, source=None):
        ...
        for i, song_info in enumerate(songs):
            ...
            singer_text = (
                ", ".join([s.get("name", "") for s in singers if isinstance(s, dict)])
                if isinstance(singers, list)
                else ""
            )
            ...
            download_btn.clicked.connect(
                lambda _, song_index=i, song_source=source: self.download_playlist_link_song(
                    song_index, song_source
                )
            )
    ...
    def search_playlist_songs_details(self, source=None):
        ...
        self.current_worker = WorkerThread(
            "search_playlist_link_songs_one_by_one",
            api=self.api,
            params={"songs": original_songs},
        )
        self.current_worker.update_signal.connect(self.handle_single_song_search_result)
        self.current_worker.error_signal.connect(self.handle_worker_error)
        self.current_worker.progress_signal.connect(self.handle_progress_update)
        self.current_worker.start()
    ...
    def handle_single_song_search_result(self, data):
        ...
        # 而是使用已完成的搜索数量计算进度
        completed_count = sum(1 for s in songs if s is not None)
        ...
        if song_info:
            ...
            # 歌曲信息
            table.setItem(index, 1, QTableWidgetItem(song_info["name"]))
            table.setItem(
                index,
                2,
                QTableWidgetItem(", ".join([s["name"] for s in song_info["singer"]])),
            )
            table.setItem(index, 3, QTableWidgetItem(song_info["album"]["name"]))

            # 时长
            duration = song_info.get("interval", 0)
            minutes, seconds = divmod(duration, 60)
            table.setItem(index, 4, QTableWidgetItem(f"{minutes:02d}:{seconds:02d}"))

            # 可用格式
            available_formats = self._get_available_formats(song_info)
            table.setItem(index, 5, QTableWidgetItem(available_formats))

            # 下载按钮
            download_cell, download_btn = self._create_download_cell_widget(table)
            download_btn.clicked.connect(
                lambda _, song_index=index, song_source=source: self.download_playlist_link_song(
                    song_index, song_source
                )
            )
